            ...
            else  //开始合并
            {
                ...
                if (NoMerge == false)
                {
                    ...
                    if (Program.isWindows)
                        driverName = exePath.Remove(exePath.IndexOf(':'));
                    Console.Title = "Done.";
                    ...
                    if (PartsCount == 1)
                    {
                        ...
                        FFmpeg.ReportFile = (Program.isWindows ? driverName + "\\:" + exePath.Remove(0, exePath.IndexOf(':') + 1).Replace("\\", "/") : exePath) + "/Logs/" + Path.GetFileNameWithoutExtension(LOGGER.LOGFILE) + fflogName;
